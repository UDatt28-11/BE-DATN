...
class RoomTypeAmenitySeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($roomTypes->isEmpty()) {
            $this->command->warn('⚠️  No room types found. Skipping room type amenities creation.');
            return;
        }
        ...
        foreach ($roomTypes as $roomType) {
            ...
            if (empty($amenityNames)) {
                // Nếu không có config, lấy 5-7 amenities ngẫu nhiên
                $allAmenities = Amenity::all();
                if ($allAmenities->isEmpty()) {
                    continue;
                }
                $selectedCount = min(rand(5, 7), $allAmenities->count());
                $selectedAmenities = $allAmenities->random($selectedCount);
            } else {
                // Tìm amenities theo tên (không filter theo property)
                $selectedAmenities = Amenity::whereIn('name', $amenityNames)->get();
            }
        ...
        $this->command->info("✅ Assigned {$totalAssigned} amenity relationships to room types");
    }
}
